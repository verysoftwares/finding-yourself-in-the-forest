...
function breathe15()
		...
		if mt%(11)==0 then
				sfx(5,
				    -5+12*2+12-place.loop+mt%6+mt%1-mt%24,
								155+1,
								2,
								0x8,
								0)
...
function gameon()
   ...
   -- Z: resolve hotspot verb
       ...
       if action then action() 
   				...
   				hotspots.i=1
       if not (focus ==lore) and not (focus==params) then focus=verbs end
   		end
   ...
   -- Z: select, action happens, focus state may change.
	...
	if place.x ==12 and place.y ==24*2 then
			if not verb_has('Reflect') and not inv_has(39) then
					softlock=true
			end
	end
	end	--if not flags["S"]
	...
	if flags[2] or pmem(10) ==1 then
			...
			iy=8
				...
				--borderlabel('Items',tx+8*10-8+8,0,8)
			--inventory items
 ...
 lorespam(function()
   if not (focus==params) then focus=verbs end
   ...
   if flags["E"] then 
	...
	if (pmem(0) ==1 or flags["P"]) and lore.msg ==nil then
			...
			borderlabel('Thoughts',8,8+12*8+8,15)
	...
	if place.x==204 and place.y==112 then
	breathe14()
	end
	...
	for i,f in ipairs(flies) do
			...
			if math.random(1,100)<=4 then
					...
					if fx>=0 and fx<12 and fy>=0 and fy<12 then
							if plantable(mget(place.x+fx,place.y+fy+12)) then
									mset(place.x+fx,place.y+fy+12,0)
							end
					end
			end
	...
	if place.loop then 
	local tw=print('Autio',8+7*8,8+8*8,3,false,1,true) 
	print('tupa',8+7*8+tw-1,8+8*8,3,false,1,true) 
	tw=print(fmt('%d km',place.loop),0,-6,3,false,1,true)
	print(fmt('%d km',place.loop),8+7*8+2*8-tw/2,8+9*8,3,false,1,true)
	end
		...
		print('w',8+29+6+12+1+5,8+43+8-1+15+2,3,false,1,true)
			...
			for i,param in ipairs(params) do
				...
				if focus ==params and i ==params.i then
					...
					local col=2
					local msg
					if i<=5 then msg=param..fmt(" (%d)",paramsus[param])
					else msg=param end

					for char=1,#(msg) do
						local mark=sub(msg,char,char)
						if mark=='(' and i<=5 then col=15 end
						print(mark,tx+(char-1)*8+cos(t*0.2+char*8)*2,ty+sin(t*0.2+char*8)*2,col)
					end
				else
					tw=print(param,tx,ty,2)
					if i<=5 then print(fmt(" (%d)",paramsus[param]),tx+tw,ty,15) end
				end
				ty=ty+8
				--if we go overboard, increment column..
			end
  ...
  for i,scr in ipairs(pg_screens) do
    ...
    if scr.place then
      ...
      draw_place(scr.place,pg_screens.camx+camw+8,4+8)
						...
						if scr.place.loop then 
						local tw=print('Autio',pg_screens.camx+camw+8+7*8,8+8*8+4,3,false,1,true) 
						print('tupa',pg_screens.camx+camw+8+7*8+tw-1,8+8*8+4,3,false,1,true) 
						tw=print(fmt('%d km',scr.place.loop),0,-6,3,false,1,true)
						print(fmt('%d km',scr.place.loop),pg_screens.camx+camw+8+7*8+2*8-tw/2,8+9*8+4,3,false,1,true)
						end
    ...
    if cntx=='hotspot resolve' then
      -- if is first occurence in pmem and..
						...
						if verb=='Retry' and #inventory>0 then
								if (place.x ==12 and place.y ==24*2) or (place.x==228 and place.y==112) or (place.x==204 and place.y==112) then
								ins(narrator_say,'Unfortunately, you could not drop your carried items here.')
								else
...
hotspot_verb_resolve ={
  ...
  ['Move'] = function()
    ...
    if not place.loop then mt=0 end
    ...
    elseif (place.x==228 and place.y==112) or (hspot.nextx==228 and hspot.nexty==112) then
      ...
      if not place.loop then if place.x==204 and place.y==112 then place.loop=1 else place.loop=12 end; place.x=hspot.nextx; place.y=hspot.nexty
      		...
      		if hspot.nexty>112 then place.loop=place.loop+1; if place.loop>12 then place.x=228; place.y=0; place.loop=nil; mt=0 end 
        elseif hspot.nexty<112 then place.loop=place.loop-1; if place.loop<1 then place.x=204; place.y=112; place.loop=nil; mt=0 end 
        end
     	end
    else
  ...
  ['Climb'] = function()
    ...
    --else      
      ...
      if place.x ==12*2+12*2 and place.y ==112 then 
        if flags["H"] then
          inform_once("<Mallard> You haven't Talked to me at all, what's wrong?")
        end
        flags["H"]=true 
      end
      fed=false
...
select_verb ={
  ...
  ['Throw'] = function()
    ...
    if (place.x==24 and place.y==24) or (place.x ==12 and place.y ==24*2) or ((place.x==228 or place.x==204) and place.y==112) then
						inform('Unable to throw here.')
						return
				end
  ...
  ['Reflect'] = function()
    ...
    if place.x ==12 and place.y ==24+24 then
      ...
      local ed={}
      if amt_thoughts() ==0 then
        for i,li in ipairs(endings[1]) do
        		ins(ed,li)
        end
      		flags['E0']=true
        --pmem(1)=1
      ...
      if amt_thoughts()>=1 then --and amt_thoughts()<=total_thoughts()-5 then
        ...
        for i,li in ipairs(endings[2]) do
        		ins(ed,li)
        end
        flags["E2"]=true; end
        if mostly_positive() then 
        --endings[4][#(endings[4])] = fmt()
        for i,li in ipairs(endings[4]) do
        		ins(ed,li)
        end
								flags["E4"]=true; end
      end
      ...
      local first_ed=true
      for i=0,6-1 do if pmem(20+i)>0 then first_ed=false; break end end
						if first_ed then
						ins(ed,'...Oh hey! Since this is your first time beating the game, I\'ve got a little reward for you.')
						ins(ed,'You\'ll start your next runs with a carrot in your inventory.')
						ins(ed,'I hope you enjoy what it brings.')
						end

      inform_multi(ed)

						if flags['E0'] then pmem(20,1) end						
						if flags['E2'] then pmem(22,1) end
						if flags['E4'] then pmem(24,1) end
  ...
  ['Plant'] = function()
    ...
    --then a place for it.
    if (place.x==204 or place.x==228) and place.y==112 then
    		inform('The earth here is barren.')
      return
    end
...
inventory_verb_resolve ={
  ...
  ['Eat'] = function()
    ...
    if item.id==135 then --carrot
    		inform('Eating a carrot gave you the energy to chop 1 time.')
      sustain('Chop',1)
    end
  ...
  ['Throw'] = function()
    ...
    if place.x ==12*2 and place.y ==0 then
      ...
      if not flags["DG"] and not done then 
        if goodfood(cur(inventory).id) and cur(inventory).id~=25 then 
          ...
          if sp ==7 or sp ==7+2*2 then
            inform("You throw a harmful item at the mallard, who nimbly dodges. They don't feel safe with you anymore.")
            flags["DG"]=true
          end
        end
...
duck_msg="It falls into the water, attracting a mallard curious for food."
...
function cycle2(tbl)
  ...
  if btnp(1) then tbl.i=tbl.i+1; if tbl.i>#tbl then tbl.i=1 end end
		if btnp(2) and tbl.i<=5 then psustain(cur(params),-1) end
		if btnp(3) and tbl.i<=5 then psustain(cur(params),1) end
end
...
function borderlabel(title,lx,ly,col)
		local tw=print(title,0,-6,1,false,1,true)
		rect(lx,ly,tw+2,8,0)
		print(title,lx+1,ly,col,false,1,true)
end
...
function mov_hotspots()
  ...
  if place.x==228 and place.y==112 then
  		ins(hotspots, {id=128,rot=0,x=5,y=0,w=2,h=2,nextx=228,nexty=112-24})
  		ins(hotspots, {id=128,rot=2,x=5,y=12-2,w=2,h=2,nextx=228,nexty=112+24})
    return
  end
  if place.x==204 and place.y==112 then
  		ins(hotspots, {id=128,rot=0,x=7,y=12-5,w=2,h=2,nextx=204,nexty=112-24})
  		ins(hotspots, {id=128,rot=2,x=5,y=12-2,w=2,h=2,nextx=228,nexty=112})
    return
  end
  ...
  if found then
  		if nextx==228 and nexty==112 and flags['Ch'] then
      ins(hotspots, {id=128,rot=0,x=8,y=3,w=2,h=2,nextx=nextx,nexty=nexty})
				elseif nextx~=228 and nexty~=112 then
...
function chop_hotspots(target_tbl)
  ...
  px=place.x; py=place.y
  if px==12 and py==0 then return end
  for my=0,12-1 do
    for mx=0,12-1 do
      sp = mget(px+mx, py+my)
      if sp ==68 then
...
-- postgame carrot
for i=0,6-1 do
		if pmem(20+i)>0 then ins(inventory,{id=135,w=2,h=2}); break end
end
...
-- 036:aa77777aaa7aaa7aa7aaaaa77aaaaaaa7aaaaaaa7aaaaaaa7aaaaaa7a7aaaa77
...
-- 162:4444444444444444444444444444444444444444444444441111111111111111
-- 163:1111111111111111111111111111111111111111111111114444444444444444
...
-- 178:4444444444444444111111111111111144444444444444441111111111111111
...
-- 190:aaaafaaaaaafafaaaaafafaafffaaaffaafaaafaafaaaaafaaffaffaaaaafaaa
-- 191:aaaafaf0aaaafaf0aaaffaa0ffffaaf0aaaaaff0aaaaffafaafffaaffffaafaa
...
-- 206:aaaaaaafaaaaaffaaaaffaafaaffaaffafaaaffaffafaaaafaaaaafffafafaaa
-- 207:faaaaaafaffafaafffaaaaf0aaafaf00aaafff00faffaaf0affaaaf0fffaaf00
...
-- 017:
-- 018:
-- 019:
-- 020:
...
-- 112:
-- 113:
-- 114:
-- 115:
-- 116:
-- 117:
-- 118:
-- 119:
-- 120:
-- 121:
-- 122:
-- 123:
